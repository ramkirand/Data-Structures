...
import lineCorporation.TreeNode;
...
public class Tree {
	...
	public static void printRightViewTree(TreeNode rootNode, int nextLevel) {
		int currentLevel = 0;
		if (rootNode == null)
			return;
	...
	// neighbourNode = (parentNighbour.getLeftChild() != null) ?
	// parentNighbour.getLeftChild()
	...
	// neighborNode = (parentNeighbor.getLeftChild() != null) ?
	// parentNeighbor.getLeftChild()
	...
	// while (temp != null) {
	// if (currentNode == null) {
	// currentNode = (temp.getLeftChild() != null) ? temp.getLeftChild() :
	// temp.getRightChild();
	// }
	...
	public static void printPathRootToLeaf(TreeNode r) {
		if (r == null)
			return;
	...
	private static void printRootToLeaf(TreeNode r, ArrayList<Integer> path) {
	...
	public static TreeNode deSerializeBinaryTree(String searialzedTree) {
		...
		node.addAll(Arrays.asList(searialzedTree.split(",")));
		TreeNode root = deSerializeBinaryTreeHelper(node);
		return root;
	}

	private static TreeNode deSerializeBinaryTreeHelper(Queue<String> node) {
		String valueOfNode = node.poll();
		if (valueOfNode.equals("X"))
			return null;
		TreeNode newNode = new TreeNode();
		newNode.setData(Integer.valueOf(valueOfNode));
		newNode.setLeftChild(deSerializeBinaryTreeHelper(node));
		newNode.setRightChild(deSerializeBinaryTreeHelper(node));
		return newNode;
	}
	...
	public static void printArrayList(ArrayList<Integer> pathList) {
		System.out.print("path: ");
		pathList.forEach(i -> System.out.print(i + " "));
		System.out.println();
	...
	// Min depth of a Binary returnsearch Tree
	public int minDepthTree(TreeNode r) {
		...
		if (r.getLeftChild() == null)
			return minDepthTree(r.getRightChild()) + 1;

		if (r.getRightChild() == null)
			return minDepthTree(r.getLeftChild()) + 1;

		return Math.min(minDepthTree(r.getRightChild()), minDepthTree(r.getLeftChild()));
	...
	public void printDiagonalMap(HashMap<Integer, ArrayList<Integer>> mapList) {
		...
		for (Entry<Integer, ArrayList<Integer>> entry : mapList.entrySet()) {
			System.out.print("Tree left Count:" + entry.getKey() + ",Tree Diagonals:" + entry.getValue().toString());
			System.out.println();
		}
	...
	public void displayDiagonalTree(Map<Integer, List<Integer>> map) {
		for (Map.Entry<Integer, List<Integer>> entry : map.entrySet()) {
			System.out.println("Level[" + entry.getKey() + "]" + ":" + Arrays.toString(entry.getValue().toArray()));
		}
	}
	...
	public static void computeDiagonalSumTree(TreeNode root, int currentDiag, Map<Integer, Integer> map) {
		if (root == null)
			return;
	...
	public static boolean bstUtil(TreeNode root, int min, int max) {
		...
		return bstUtil(root.getLeftChild(), min, root.getData() - 1)
				&& bstUtil(root.getRightChild(), root.getData() + 1, max);
	...
	public static int rightBranchSum(TreeNode root) {
		if (root == null)
			return 0;
		return root.getData() + rightBranchSum(root.getRightChild());
	}

	public static int leftBranchSum(TreeNode root) {
		if (root == null)
			return 0;
		return root.getData() + leftBranchSum(root.getLeftChild());
	}

	public static int treeSum(TreeNode root) {
		if (root == null)
			return 0;
		return root.getData() + leftBranchSum(root.getLeftChild()) + rightBranchSum(root.getRightChild());
	}

	public static void printRightViewTree2(TreeNode r) {
		if (r == null)
			return;
	...
	public static void main(String[] args) {
		...
		T.add(4);
		T.add(7);
		// printRightViewTree2(T.root);
		System.out.println(serializeBinaryTree(T.root));
		System.out.println("\nDe Serialized BinaryTree\n");
		T.preOrderTraversal(deSerializeBinaryTree(serializeBinaryTree(T.root)));
		// printRightViewTree(T.root, 1);
		System.out.println();
		// printPathRootToLeaf(T.root);
	...
	private static void displayMap(Map<Integer, Integer> diagSumMap) {
		diagSumMap.forEach((k, v) -> System.out.println("Level" + k + "," + v));
	}
